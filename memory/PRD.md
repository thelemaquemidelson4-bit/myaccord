...
- 2026-06-21: Correction visibilité logo Myaccord (onboarding/auth). Fonctions LIVE (WebRTC) implémentées: appels audio/vidéo 1:1 (bouton dans le chat + message d'invitation "Rejoindre"), diffusion en direct 1-à-N (onglet "Direct" + "Passer en direct"). Signalisation via WebSocket FastAPI (/api/ws/signal/{room_id}, auth par token en query), STUN Google, react-native-webrtc + config plugin. Web = fallback "non disponible" (nécessite build natif). Backend REST+WS vérifiés manuellement.
- 2026-07-26: Fix déploiement (alias metro event-target-shim/index → root, config-plugin webrtc aligné SDK54 v13). Logo texte "Myaccord" (accueil/onboarding/auth), suppression des images de logo dans les écrans. Nouveau logo (épingle M) comme icône + splash. Nouveau rôle "fan" (Supporter): parcourt athlètes/offres, regarde les directs, messagerie — sans publier d'offres ni postuler. Onboarding 3 cartes, toggle Athlètes/Offres à l'accueil, profil minimal. Backend 21/21 + Frontend 24/24 tests (iteration_7).
